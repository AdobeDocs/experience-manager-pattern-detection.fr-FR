...
`ACV` (Programme de validation du contenu des ressources) : identifie les nœuds obligatoires manquants et les violations dans le contenu des ressources. Cela peut entraîner l’échec de certaines fonctionnalités d’Assets sur Experience Manager as a Cloud Service.
...
* `psb.file.large` : identifie les fichiers PSB volumineux (dc:format : application/vnd.3gpp.pic-bw-small) d’une taille supérieure à 2 Go.
...
* Un grand nombre de descendants sous le nœud de métadonnées peut ralentir le téléchargement de dossiers composés de ressources qui enfreignent cette règle.
