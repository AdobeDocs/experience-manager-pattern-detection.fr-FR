...
>title="Problème de configuration de la mise à niveau"
...
* `org.apache.sling.commons.log.LogManager.factory.config` : déterminez si la propriété `org.apache.sling.commons.log.file` des enregistreurs personnalisés pointe vers autre chose que `logs/error.log` fichier .
...
* La modification ou la suppression de configurations peut entraîner les problèmes suivants :
   ...
   * Les valeurs des `com.day.cq.commons.impl.ExternalizerImpl` de configuration d’Externalizer sont définies avec les variables d’environnement de Cloud Manager dans AEM as a Cloud Service.
